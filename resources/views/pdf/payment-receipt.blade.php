...
<body>
    ...
    <p style="margin-top: 20px;">
        <strong>No.</strong> {{ $payment->receipt_number }}<br>
        <strong>Date</strong> {{ $payment->payment_date?->format('d M Y') }}<br>
        <strong>Received from</strong> {{ $customer->name ?? '—' }}
    ...
    <div class="box">
        <div class="row">
            ...
            <p style="margin: 0;">Invoice <strong>{{ $invoice->invoice_number }}</strong></p>
        </div>
        ...
        <p class="muted" style="margin: 0;">
            {{ \App\Support\AmountInWords::format((float) $payment->amount, $invoice->currency ?? 'MYR') }}
        </p>
    </div>
</body>
</html>
