...
export default function NavSidebar({ sidebarOpen, onSidebarOpenChange }) {
    const page = usePage();
    ...
    const { auth, company_flags } = page.props;
    const url = page.url;
    ...
    const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
        ...
        try { return window.localStorage.getItem('bukucloud.sidebarCollapsed') === '1'; } catch { return false; }
    });
    ...
    const persistSidebarCollapsed = (next) => {
        ...
        setRailTip(null);
        try { window.localStorage.setItem('bukucloud.sidebarCollapsed', next ? '1' : '0'); } catch {}
    };

    const showRailTip = (label, event, subtitle = '') => {
        if (!sidebarCollapsed) return;
        const rect = event.currentTarget.getBoundingClientRect();
        setRailTip({ label, subtitle: subtitle || '', top: rect.top + rect.height / 2, left: rect.right + 10 });
    };
    ...
    const toggleGroup = (groupName) => {
        setOpenGroups((prev) => {
            ...
            const currentlyOpen = isSubgroup ? prev[groupName] !== false : Boolean(prev[groupName]);
            return { ...prev, [groupName]: !currentlyOpen };
        });
    };
    ...
    useEffect(() => {
        ...
        navConfig.forEach((section) => {
            ...
            if (hasActive) initialOpenGroups[section.group] = true;
        ...
        if (isRouteActive('admin.tenants.index') || isRouteActive('admin.plans.index') || isRouteActive('admin.self-hosted.index') || isRouteActive('admin.platform.show') || isRouteActive('admin.users.index') || isRouteActive('admin.audit-logs.index') || isRouteActive('admin.branding.edit') || isRouteActive('admin.ocr.edit')) initialOpenGroups['Admin'] = true;
        if (isRouteActive('settings.company') || isRouteActive('settings.changelog') || isRouteActive('settings.team.index') || isRouteActive('audit-logs.index') || isRouteActive('settings.plan.index') || isRouteActive('settings.integrations.index') || isRouteActive('audit.index') || url.startsWith('/audit')) {
                    ...
                    {!isAdmin && (
                        <div className="mb-2">
                            ...
                            <div className={`mt-1 space-y-0.5 overflow-hidden transition-all duration-300 ${openGroups['Company'] || sidebarCollapsed ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                                ...
                                <Link
                                    href={getSafeRoute('settings.changelog')}
                                    className={linkClasses(isRouteActive('settings.changelog'), false, sidebarCollapsed)}
                                    {...railProps("What's new", 'Release notes — fixes and features')}
                                >
                                    <span className={iconWrapClasses(isRouteActive('settings.changelog'))}>
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                        </svg>
                                    </span>
                                    <span className={navLabelClass(sidebarCollapsed)}>What&apos;s new</span>
                                </Link>
