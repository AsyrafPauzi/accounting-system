...
export default function Index({ auth, customers, filters = {}, base_currency = 'MYR' }) {
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-2xl lg:text-3xl font-display font-medium text-ink tracking-tight">Customer Statements</h2>
                        <p className="text-ink-muted text-sm font-medium mt-1">Pick a customer to see all invoices and payments over a date range</p>
                    </div>
                </div>
            }
        >
                ...
                <div className="bg-surface rounded-2xl border border-border-warm shadow-sm overflow-hidden">
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-cream/80 border-b border-border-warm text-[10px] font-display font-medium text-ink-muted uppercase tracking-widest">
                                    ...
                                    <th className="px-6 py-3 text-center">Open invoices</th>
                                    ...
                                    <tr key={c.id} className="hover:bg-cream/40 transition-colors">
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="flex items-center gap-2 mt-0.5 text-[11px] text-ink-muted">
                                                ...
                                                {c.tin && <span className="font-mono">· TIN {c.tin}</span>}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold ${c.outstanding_invoices_count > 0 ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300' : 'bg-surface-alt text-ink-muted'}`}>
                                                {c.outstanding_invoices_count || 0}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right font-mono tabular-nums">
                                            <span className={Number(c.outstanding_amount || 0) > 0 ? 'text-terracotta font-semibold' : 'text-ink-muted'}>
                                                {formatCurrency(c.outstanding_amount || 0, base_currency)}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <Link
                                                href={route('customer-statements.show', c.id)}
                                    ...
                                    <tr>
                                        <td colSpan="4" className="px-6 py-16 text-center">
                                            <div className="flex flex-col items-center gap-3 text-ink-muted">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-ink">No customers found</p>
                                                    <p className="text-sm mt-1">Add customers under Sales (Revenue) → Customers, then come back here.</p>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    {customers?.last_page > 1 && (
                        <div className="px-6 py-4 border-t border-border-warm flex items-center justify-between text-xs text-ink-muted">
                            <span>Showing {customers.from || 0}–{customers.to || 0} of {customers.total}</span>
                            <div className="flex items-center gap-2">
                                {customers.links?.filter(l => l.url).map((link, idx) => (
                                    <button
                                        key={idx}
                                        type="button"
