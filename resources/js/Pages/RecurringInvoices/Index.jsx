...
export default function Index({ auth, templates, filters = {}, counts = {} }) {
    ...
    const Actions = ({ template: t }) => (
        <RowActionsMenu items={[
            { label: 'Open', href: route('recurring-invoices.edit', t.id), icon: <ActionIcons.Open />, show: permissions.includes('recurring-invoices.edit') },
            { label: 'Generate now', icon: <ActionIcons.Lightning />, show: permissions.includes('recurring-invoices.run') && t.is_active, onClick: () => handleRunNow(t) },
            { label: t.is_active ? 'Pause' : 'Resume', show: permissions.includes('recurring-invoices.edit'), onClick: () => handleToggle(t) },
            { label: 'Delete', icon: <ActionIcons.Trash />, danger: true, show: permissions.includes('recurring-invoices.delete'), onClick: () => handleDelete(t) },
        ]} />
    );
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                    <div>
                        <h2 className="text-xl sm:text-2xl lg:text-3xl font-display font-medium text-ink tracking-tight">Recurring Invoices</h2>
                        <p className="text-ink-muted text-sm font-medium mt-1">Templates that generate draft invoices on a schedule</p>
                    </div>
                    {permissions.includes('recurring-invoices.create') && (
                        <Link href={route('recurring-invoices.create')} className="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl font-semibold text-white bg-terracotta hover:bg-terracotta shadow-lg transition-all duration-200">
            ...
            <div className="space-y-4 sm:space-y-6 min-w-0">
                ...
                <div className="bg-surface rounded-2xl border border-border-warm/80 shadow-sm overflow-hidden">
                    <IndexFilterBar
                        ...
                        searchPlaceholder="Search by template or customer..."
                        status={statusFilter}
                        statuses={STATUSES}
                                ...
                                {items.length > 0 ? items.map((t) => (
                                    <tr key={t.id} className={`border-b border-border-warm last:border-0 hover:bg-cream/80 transition-colors ${!t.is_active ? 'opacity-60' : ''}`}>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                            <Link href={route('recurring-invoices.edit', t.id)} className="block group/link">
                                                <span className="font-semibold text-ink group-hover/link:text-terracotta">{t.name || 'Untitled template'}</span>
                                                <p className="text-xs text-ink-muted mt-0.5">{cadenceLabel(t)}</p>
                                            </Link>
                                        </td>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                            <div className="font-medium text-ink">{t.customer?.name || '—'}</div>
                                            <p className="text-xs text-ink-muted truncate max-w-[140px] sm:max-w-none">{t.customer?.email || 'No email'}</p>
                                        </td>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                            ...
                                            {t.last_generated_invoice && (
                                                <p className="text-[10px] text-ink-muted mt-1">Last: {t.last_generated_invoice.invoice_number}</p>
                                            )}
                                        </td>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                            {t.is_active ? (
                                    ...
                                    <tr>
                                        <td colSpan={6} className="px-6 py-16 text-center text-ink-muted text-sm">
                                            {totalCount === 0 ? 'No recurring invoices yet. Create your first template to get started.' : 'No templates match your filters.'}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                            ...
                            <div key={t.id} className={`p-4 ${!t.is_active ? 'opacity-60' : ''}`}>
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0 flex-1">
                                        <Link href={route('recurring-invoices.edit', t.id)} className="font-semibold text-ink hover:text-terracotta">{t.name || 'Untitled template'}</Link>
                                        <p className="text-xs text-ink-muted mt-0.5">{t.customer?.name || '—'}</p>
