        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                    <div>
                        <h2 className="text-2xl lg:text-3xl font-display font-medium text-ink tracking-tight">Batch invoices</h2>
                        <p className="text-ink-muted text-sm font-medium mt-1">
                            Each row creates one draft invoice with a single line — post and email from the invoice when ready
                        </p>
                    </div>
            ...
            >
                ...
                <div className="bg-surface rounded-2xl border border-border-warm/80 shadow-sm overflow-hidden">
                    <div className="px-4 sm:px-5 py-3 border-b border-border-warm bg-cream/50 flex items-center justify-between gap-3">
                        <div>
                            <h3 className="text-sm font-semibold text-ink">Invoice rows</h3>
                            <p className="text-xs text-ink-muted mt-0.5">Customer, description, qty, price, and tax per draft</p>
                        </div>
                        <button
                            type="button"
                            onClick={addRow}
                        ...
                        {data.rows.map((row, i) => (
                            <div key={i} className="p-4 sm:p-5 space-y-3">
                                <div className="flex items-center justify-between gap-2">
                                    <span className="text-[10px] font-semibold uppercase tracking-widest text-ink-muted">Invoice {i + 1}</span>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button
                                            ...
                                            aria-label={`Remove invoice row ${i + 1}`}
                                        >
                                            Remove
                                        </button>
                                    </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-1">
                    <p className="text-xs text-ink-muted">
                        Drafts appear on Invoices. Post to the ledger and email from each invoice show page.
                    </p>
                    <button
                        type="button"
                        onClick={addRow}
