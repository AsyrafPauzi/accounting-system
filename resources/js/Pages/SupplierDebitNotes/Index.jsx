...
const SEARCH_KEYS = ['sdn_number', 'supplier_name'];
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                <div>
                    <h2 className="text-2xl lg:text-3xl font-display font-medium text-ink tracking-tight">Supplier debit notes</h2>
                    <p className="text-ink-muted text-sm font-medium mt-1">Additional charges from your suppliers</p>
                </div>
                {auth.permissions.includes('bills.create') && (
                    <Link href={route('supplier-debit-notes.create')} className="inline-flex items-center px-4 py-2.5 rounded-xl bg-terracotta text-white text-sm font-semibold">
                        New debit note
                    </Link>
                )}
            </div>
            ...
            <div className="space-y-6">
                <div className="bg-terracotta text-white rounded-2xl p-6 shadow-lg">
                    <span className="text-[10px] font-semibold uppercase tracking-widest opacity-90">Total debit notes</span>
                            ...
                            <tbody>
                                {filters.items.map((note) => (
                                    <tr
                                        ...
                                        onClick={() => router.visit(route('supplier-debit-notes.show', note.id))}
                                    >
                                        <td className="px-6 py-4">
                                            <Link
                                                href={route('supplier-debit-notes.show', note.id)}
                                        ...
                                        <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                            <RowActionsMenu items={[
                                                { label: 'Open', href: route('supplier-debit-notes.show', note.id), icon: <ActionIcons.Open /> },
                                                { label: 'Download PDF', href: route('supplier-debit-notes.pdf', note.id), external: true, icon: <ActionIcons.Pdf /> },
                                            ]} />
                                        </td>
                                    </tr>
                                ))}
                                {filters.items.length === 0 && (
                                    <tr><td colSpan={4} className="px-6 py-16 text-center text-ink-muted">{filters.searchInput || filters.status ? 'No supplier debit notes match your filters.' : 'No supplier debit notes yet.'}</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
