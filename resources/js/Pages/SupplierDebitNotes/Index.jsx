...
export default function Index({ auth, notes = [] }) {
    ...
    const filteredNotes = notes.filter((note) =>
        (note.sdn_number || '').toLowerCase().includes(search.toLowerCase())
                            ...
                            <tbody>
                                {filteredNotes.map((note) => (
                                    <tr key={note.id} className="border-t border-border-warm hover:bg-cream/80">
                                        <td className="px-6 py-4 font-semibold text-ink">{note.sdn_number}</td>
                                        ...
                                        <td className="px-6 py-4 text-right"><Link className="text-xs font-semibold text-terracotta" href={route('supplier-debit-notes.show', note.id)}>Open →</Link></td>
                                    </tr>
                                ))}
                                {filteredNotes.length === 0 && (
                                    <tr><td colSpan={4} className="px-6 py-16 text-center text-ink-muted">{search ? 'No supplier debit notes match your search.' : 'No supplier debit notes yet.'}</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
